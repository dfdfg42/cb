...
async function run() {
  // Import TypeScript source (will work when running with ts-node/esm loader)
  const { DamageCalculator } = await import(path.join(__dirname, '..', 'src', 'game', 'DamageCalculator.ts'));
  const { AttributeType } = await import(path.join(__dirname, '..', 'src', 'types', 'gameTypes.ts'));
  ...
  console.log('Running compiled DamageCalculator tests...');
