...
- [x] 튕기기 카드
  ...
  - [x] 새로운 방어자 지정
  - [ ] 되받아치기/튕기기 체인 처리 보강
    - [ ] 방어자가 '되받아치기' 사용 시 원래 공격자에게 공격이 되돌아가도록 하고, 원래 공격자는 해당 공격을 방어해야 함
    - [ ] 되받아치기나 튕기기로 인해 발생한 공격도 다시 되받아치기/튕기기를 사용할 수 있도록 연쇄 처리를 올바르게 관리(턴 스택/대응 스택)
    - [ ] 튕기기는 랜덤 타겟 선택 시 자신 포함 여부와 생존자 필터를 명확히 하고, 서버에서 권한을 가진 상태로 선택하도록 구현
    - [ ] 관련 파일: `src/game/TurnManager.ts`, `src/game/GameManager.ts`, `server/server.ts`
- [x] 방어 확정 기능
- [x] 멀티플레이어 방어 동기화 전송
...
- [ ] 정신력 데미지 적용
- [ ] 데미지 애니메이션 효과
 - [ ] 데미지 입을 때 공격자/피해자 시각화 구현
   - [ ] 데미지 발생 시 "누가 누구를 공격해서 누가 몇 데미지를 입었다"를 화면에 표시 (참고: 이미지 2)
   - [ ] 반사/튕김/회복 등 특수 효과도 시각적으로 구분하여 표시(예: '반사', '튕김', '+HP')
   - [ ] 관련 파일: `src/ui/CombatUI.ts`, `src/ui/PlayerComponent.ts`, `src/main.ts`
